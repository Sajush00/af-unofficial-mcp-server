"""Tests for occupancy, forecast, and the go-now verdict."""
...
def test_occupancy_reports_live_count_and_rest_of_day(fake_api, auth_env):
    ...
    assert result["busyPercent"] == 50.0
...
def test_occupancy_without_live_count(fake_api, auth_env):
    ...
    assert result["busyPercent"] is None
...
def test_verdict_bands(fake_api, auth_env, current, typical, verdict):
    fake_api.route("my-gym", home_gym_payload())
    fake_api.route(
        "busy-meter",
        busy_meter(
            current=current,
            days=week({"Wednesday": [("11:00", typical)]}),
        ),
    )
    result = occupancy.go_now_verdict(now=WEDNESDAY_11)
    assert result["verdict"] == verdict
    assert result["typicalCount"] == typical


def test_verdict_without_live_count(fake_api, auth_env):
    fake_api.route("my-gym", home_gym_payload())
    fake_api.route("busy-meter", busy_meter(current=None, days=[]))
    result = occupancy.go_now_verdict(now=WEDNESDAY_11)
